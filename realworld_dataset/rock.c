...
repeat:
	while (rs.len > 2) { /* There may be one byte for padding somewhere */
		...
		case SIG('R', 'E'):
			...
			goto out;
		case SIG('C', 'L'):
/*#MFLAWTAG*/			ISOFS_I(inode)->i_first_extent =
/*#MFLAWTAG*/			    isonum_733(rr->u.CL.location);
/*#MFLAWTAG*/			reloc =
/*#MFLAWTAG*/			    isofs_iget(inode->i_sb,
/*#MFLAWTAG*/				       ISOFS_I(inode)->i_first_extent,
/*#MFLAWTAG*/				       0);
			if (IS_ERR(reloc)) {
				ret = PTR_ERR(reloc);
...
/*#MFLAWTAG*/int parse_rock_ridge_inode(struct iso_directory_record *de, struct inode *inode)
{
/*#MFLAWTAG*/	int result = parse_rock_ridge_inode_internal(de, inode, 0);
...
/*#MFLAWTAG*/		result = parse_rock_ridge_inode_internal(de, inode, 14);
	}
	return result;
}
