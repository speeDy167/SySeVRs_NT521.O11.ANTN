...
static int
/*#MFLAWTAG*/dissect_websocket_payload(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, proto_tree *ws_tree, guint8 opcode, guint payload_length, guint8 mask, const guint8* masking_key)
{
  guint               offset = 0;
...
static int
/*#MFLAWTAG*/dissect_websocket(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void *data _U_)
{
  proto_item   *ti, *ti_len;
/*#MFLAWTAG*/  guint8        fin, opcode, mask;
/*#MFLAWTAG*/  guint         length, short_length, payload_length, recurse_length;
/*#MFLAWTAG*/  guint         payload_offset, mask_offset, recurse_offset;
  proto_tree   *ws_tree;
  const guint8 *masking_key = NULL;
  tvbuff_t     *tvb_payload;

/*#MFLAWTAG*/  length = tvb_length(tvb);
/*#MFLAWTAG*/  if (length < 2) {
/*#MFLAWTAG*/    pinfo->desegment_len = 2;
/*#MFLAWTAG*/    return 0;
/*#MFLAWTAG*/  }

  short_length = tvb_get_guint8(tvb, 1) & MASK_WS_PAYLOAD_LEN;
  if (short_length == 126) {
/*#MFLAWTAG*/    if (length < 2+2) {
/*#MFLAWTAG*/      pinfo->desegment_len = 2+2;
/*#MFLAWTAG*/      return 0;
/*#MFLAWTAG*/    }
    payload_length = tvb_get_ntohs(tvb, 2);
/*#MFLAWTAG*/    mask_offset = 2+2;
/*#MFLAWTAG*/  }
/*#MFLAWTAG*/  else if (short_length == 127) {
/*#MFLAWTAG*/    if (length < 2+8) {
/*#MFLAWTAG*/      pinfo->desegment_len = 2+8;
/*#MFLAWTAG*/      return 0;
/*#MFLAWTAG*/    }
    /* warning C4244: '=' : conversion from 'guint64' to 'guint ', possible loss of data */
    payload_length = (guint)tvb_get_ntoh64(tvb, 2);
/*#MFLAWTAG*/    mask_offset = 2+8;
/*#MFLAWTAG*/  }
/*#MFLAWTAG*/  else{
    payload_length = short_length;
/*#MFLAWTAG*/    mask_offset = 2;
  }

  /* Mask */
/*#MFLAWTAG*/  mask = (tvb_get_guint8(tvb, 1) & MASK_WS_MASK) >> 4;
  payload_offset = mask_offset + (mask ? 4 : 0);

/*#MFLAWTAG*/  if (payload_offset + payload_length < payload_length) {
/*#MFLAWTAG*/    /* Integer overflow, which means the packet contains a ridiculous
/*#MFLAWTAG*/     * payload length. Just take what we've got available.
/*#MFLAWTAG*/     * See bug https://bugs.wireshark.org/bugzilla/show_bug.cgi?id=8448 */
/*#MFLAWTAG*/    payload_length = tvb_reported_length_remaining(tvb, payload_offset);
/*#MFLAWTAG*/  }

/*#MFLAWTAG*/  if (length < payload_offset + payload_length) {
/*#MFLAWTAG*/    /* XXXX Warning desegment_len is 32 bits */
/*#MFLAWTAG*/    pinfo->desegment_len = payload_offset + payload_length - length;
/*#MFLAWTAG*/    return 0;
/*#MFLAWTAG*/  }

/*#MFLAWTAG*/  /* We've got the entire message! */
...
/*#MFLAWTAG*/  /* Call this function recursively, to see if we have enough data to parse another websocket message */

/*#MFLAWTAG*/  recurse_offset = payload_offset + payload_length;
/*#MFLAWTAG*/  if (length > recurse_offset) {
/*#MFLAWTAG*/    recurse_length = dissect_websocket(tvb_new_subset_remaining(tvb, recurse_offset), pinfo, tree, data);
/*#MFLAWTAG*/    if (pinfo->desegment_len)
/*#MFLAWTAG*/      pinfo->desegment_offset += recurse_offset;

/*#MFLAWTAG*/    return recurse_offset + recurse_length;
  }

/*#MFLAWTAG*/  return recurse_offset;
}
