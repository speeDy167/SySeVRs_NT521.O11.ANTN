                                                           ...
                                                           int vr_type)
{
    ...
    for (pass = 0; pass <= vr->maxpass; ++pass) { // FIXME OPTIMIZE?
        ...
        for (partition_count = 0; partition_count < ptns_to_read;) {  // SPEC        error
            if (!pass) {
                int ret;
/*#MFLAWTAG*/                if ((ret = setup_classifs(vc, vr, do_not_decode, ch_used, partition_count)) < 0)
                    return ret;
            }
